...
class ControllableSimulation(Simulation): #TODO: move interface to separate class
    ...
    def apply_random_meta_control(self):
        ...
        if falsification_config.env_mutation_breadth == ENV_MUTATION_BREADTH.RANDOM:
            mutation_breadth = random.randint(1,len(simulation_copy.obstacles_in_WF))
        elif falsification_config.env_mutation_breadth == ENV_MUTATION_BREADTH.CONSTANT:
            mutation_breadth = falsification_config.number_of_obstacles_to_mutate

        # remove old obstacles
        history_compromise_timestamp = inf
        obstacles_removed_in_WF = []
        if falsification_config.env_mutation_type == ENV_MUTATION_TYPE.REMOVE \
            or falsification_config.env_mutation_type == ENV_MUTATION_TYPE.REPLACE:
        ...
        # add new obstacles
        obstacles_added = []
        if falsification_config.env_mutation_type == ENV_MUTATION_TYPE.ADD \
            or falsification_config.env_mutation_type == ENV_MUTATION_TYPE.REPLACE:
            # limited depth replacement
            if falsification_config.env_mutation_depth == ENV_MUTATION_DEPTH.LOCAL_PERTUBATION:
                ...
                for obstacle in obstacles_removed_in_WF:
                    ...
                    while not new_obstacle_is_valid:
                        new_x = normal(loc=obstacle[0], scale=falsification_config.obs_perturbation_stdv)
                        new_y = normal(loc=obstacle[1], scale=falsification_config.obs_perturbation_stdv)
        ...
        if falsification_config.incremental_simulation == INCREMENTAL_SIM.NO:
            simulation_copy.rollback(to_timestamp=0)
            control_steps_saved = 0
        elif falsification_config.incremental_simulation == INCREMENTAL_SIM.YES:
    ...
    def _add_obstacles_to_env_and_calc_HCTS(self, suggested_number_of_obstacles, obstacles=None):
        ...
        if obstacles is not None:
            ...
            num_of_obs_to_add = min([max([0, falsification_config.max_number_of_obstacles-len(self.obstacles_in_WF)]),\
